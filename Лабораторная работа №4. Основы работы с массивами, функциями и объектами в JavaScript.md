 ...
 * Класс, представляющий базовый предмет в инвентаре. */class Item {  
    /**  
    ...
    /**  
     * Получить информацию о предмете.     * @returns {string} Описание предмета.  
     */    getInfo() {  
        return `[Item] ${this.name} | Вес: ${this.weight} | Редкость: ${this.rarity}`;  
    }  
  
    /**  
     * Изменить вес предмета.     * @param {number} newWeight - Новый вес.  
     */    setWeight(newWeight) {  
...
/**  
 * Класс Оружия, расширяющий базовый предмет. * @extends Item  
 */  
class Weapon extends Item {  
    ...
    /**  
     * Использовать оружие (уменьшает прочность).     */    use() {  
    ...
    /**  
     * Восстановить прочность до максимума.     */    repair() {  
        this.durability = 100;  
        console.log(`${this.name} отремонтировано.`);  
    }  
  
    /**  
     * Переопределение метода для вывода характеристик оружия.     * @returns {string}  
     */  
    getInfo() {  
...
/**  
 * Базовая функция-конструктор предмета. * @constructor  
 */  
...
/**  
 * Функция-конструктор оружия. * @constructor  
 */  
...
```
## Контрольные вопросы
### 1. `this` в методах класса
- Ссылается на **текущий экземпляр** объекта (созданный через `new`).
- Позволяет обращаться к свойствам и другим методам этого же объекта.
### 2. Модификатор `#`
- Создает **приватные** поля и методы.
- Доступны **только внутри** тела класса; извне или из наследников обратиться нельзя (будет ошибка).
### 3. Классы vs Конструкторы
- **Классы:** Обязателен `new`, не всплывают (no hoisting), всегда в `strict mode`, методы не перечисляются.
- **Конструкторы:** Можно вызвать как функцию, всплывают, методы по умолчанию перечисляемы.